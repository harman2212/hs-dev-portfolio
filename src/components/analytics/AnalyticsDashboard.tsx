        ...
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Real-time visitor tracking &amp; insights</p>
            </div>
          </div>
          <Button
            variant="outline"
      ...
      <div className="container mx-auto px-4 py-6">
          ...
          <>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground flex items-center gap-1">
                        <Clock className="h-3 w-3" /> {stats.todayVisitors} today
                      </p>
                    </div>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground flex items-center gap-1">
                        <Clock className="h-3 w-3" /> {stats.todayPageViews} today
                      </p>
                    </div>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">By fingerprint</p>
                    </div>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold">{stats.countries.length}</p>
                      <p className="text-xs text-muted-foreground">Regions detected</p>
                    </div>
            ...
            <Card className="mb-6">
              <CardHeader className="pb-2">
                <CardTitle className="text-base">Traffic Overview (Last 7 Days)</CardTitle>
                <CardDescription>Daily visitors and page views</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={stats.dailyVisitors.map((d, i) => ({
                      date: formatDate(d.date),
                      visitors: d.count,
                      pageViews: stats.dailyPageViews[i]?.count || 0,
                    }))}>
                      <defs>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              ...
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-base flex items-center gap-2">
                    <ExternalLink className="h-4 w-4" /> Top Pages
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2">
                    {stats.pages.slice(0, 5).map((p, i) => (
                      <div key={i} className="flex items-center justify-between">
              ...
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-base flex items-center gap-2">
                    <Monitor className="h-4 w-4" /> Top Browsers
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2">
              ...
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-base flex items-center gap-2">
                    <Search className="h-4 w-4" /> Top Referrers
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2">
                    {stats.referrers.slice(0, 5).map((r, i) => (
                      <div key={i} className="flex items-center justify-between">
                        <div className="flex items-center gap-2 min-w-0">
                          <span className="text-xs text-muted-foreground w-4">{i + 1}</span>
                          <span className="text-sm truncate">{r.name || 'Direct'}</span>
                        </div>
            ...
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardDescription>Latest tracked visitor sessions</CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b text-muted-foreground">
                        ...
                        <th className="text-left py-2 px-2 font-medium hidden sm:table-cell">Device</th>
                        <th className="text-left py-2 px-2 font-medium hidden md:table-cell">Browser</th>
                        <th className="text-left py-2 px-2 font-medium hidden lg:table-cell">OS</th>
                        <th className="text-left py-2 px-2 font-medium hidden lg:table-cell">Page</th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr key={v.id} className="border-b last:border-0 hover:bg-muted/50">
                          ...
                          <td className="py-2 px-2 hidden sm:table-cell">
                            <div className="flex items-center gap-1">
                              {v.device === 'Mobile' ? <Smartphone className="h-3 w-3" /> : <Monitor className="h-3 w-3" />}
                              {v.device || '-'}
                            </div>
                          </td>
                          <td className="py-2 px-2 hidden md:table-cell">{v.browser || '-'}</td>
                          <td className="py-2 px-2 hidden lg:table-cell">{v.os || '-'}</td>
                          <td className="py-2 px-2 hidden lg:table-cell text-xs text-muted-foreground max-w-[150px] truncate">
                            {v.page || '-'}
                          </td>
                        </tr>
                      ))}
                      {stats.recentVisitors.length === 0 && (
                        <tr>
                          <td colSpan={6} className="py-6 text-center text-muted-foreground">
                            No visitors tracked yet. Visit the site to start tracking!
                          </td>
                        </tr>
                      )}
                    </tbody>
          ...
          <>
            <div className="mb-4 flex gap-2">
              <div className="relative flex-1 max-w-md">
                ...
                <Input
                  placeholder="Search by IP, country, browser, OS, device..."
                  className="pl-9"
                  value={search}
                  onChange={(e) => { setSearch(e.target.value); setVisitorsPage(1) }}
                />
              </div>
            </div>
            <Card>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b bg-muted/50">
                        ...
                        <th className="text-left py-3 px-4 font-medium">Fingerprint</th>
                        ...
                        <th className="text-left py-3 px-4 font-medium hidden sm:table-cell">Device</th>
                        <th className="text-left py-3 px-4 font-medium hidden lg:table-cell">Browser / OS</th>
                        <th className="text-left py-3 px-4 font-medium hidden xl:table-cell">Referrer</th>
                        <th className="text-left py-3 px-4 font-medium hidden xl:table-cell">Screen</th>
                      </tr>
                    </thead>
                    <tbody>
                      {visitors.map((v, i) => (
                      ...
                      {visitors.length === 0 && (
                        <tr><td colSpan={9} className="py-12 text-center text-muted-foreground">No visitors found</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
        ...
        {activeTab === 'charts' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-base">Browsers</CardTitle></CardHeader>
              <CardContent>
                {stats.browsers.length > 0 ? (
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={stats.browsers} dataKey="count" nameKey="name" cx="50%" cy="50%" outerRadius={80}
                          label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={false}>
                          {stats.browsers.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                        </Pie>
                        <Tooltip />
                      </PieChart>
                    </ResponsiveContainer>
            ...
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-base">Operating Systems</CardTitle></CardHeader>
              <CardContent>
                {stats.os.length > 0 ? (
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={stats.os} dataKey="count" nameKey="name" cx="50%" cy="50%" outerRadius={80}
                          label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={false}>
                          {stats.os.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                        </Pie>
                        <Tooltip />
                      </PieChart>
                    </ResponsiveContainer>
            ...
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-base">Page Views</CardTitle></CardHeader>
              <CardContent>
                {stats.pages.length > 0 ? (
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={stats.pages}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#333" />
            ...
            <Card className="md:col-span-2">
              <CardHeader className="pb-2"><CardTitle className="text-base">Visitors by Country</CardTitle></CardHeader>
              <CardContent>
                  ...
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={stats.countries} layout="vertical">
                        ...
                        <YAxis type="category" dataKey="name" tick={{ fontSize: 12 }} stroke="#666" width={100} />
                        <Tooltip contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '8px', color: '#fff' }} />
